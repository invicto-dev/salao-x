...
const Agendamentos = () => {
  ...
  const funcionarios = [
    ...
    { id: '2', nome: 'Ana Costa', especialidades: ['Coloração', 'Tratamentos'] },
    { id: '3', nome: 'Beatriz Lima', especialidades: ['Manicure', 'Pedicure'] },
  ];

  const servicos = [
    { id: '1', nome: 'Corte Feminino', duracao: 60, preco: 50.00 },
    { id: '2', nome: 'Coloração', duracao: 120, preco: 80.00 },
    { id: '3', nome: 'Manicure', duracao: 45, preco: 25.00 },
    { id: '4', nome: 'Escova', duracao: 45, preco: 30.00 },
    { id: '5', nome: 'Pedicure', duracao: 60, preco: 35.00 },
  ];

  const getStatusConfig = (status: string) => {
    const configs = {
      'agendado': { color: 'blue', icon: <CalendarIcon size={12} />, text: 'Agendado' },
      'confirmado': { color: 'green', icon: <CheckCircle size={12} />, text: 'Confirmado' },
      'em-andamento': { color: 'orange', icon: <Clock size={12} />, text: 'Em Andamento' },
      'concluido': { color: 'green', icon: <CheckCircle size={12} />, text: 'Concluído' },
      'cancelado': { color: 'red', icon: <XCircle size={12} />, text: 'Cancelado' }
    };
  ...
  const alterarStatus = (agendamentoId: string, novoStatus: string) => {
    message.success(`Status alterado para: ${getStatusConfig(novoStatus).text}`);
  };

  const verificarConflito = (funcionarioId: string, data: Dayjs, hora: Dayjs, duracao: number, agendamentoId?: string) => {
    const agendamentosData = getAgendamentosPorData(data).filter(ag => 
    ...
    return agendamentosData.some(ag => {
      const agHoraInicio = parseInt(ag.hora.split(':')[0]) * 60 + parseInt(ag.hora.split(':')[1]);
  ...
  const handleSubmit = (values: any) => {
    ...
    if (servico) {
      ...
      if (temConflito) {
        message.error('Conflito de horário! Este funcionário já possui agendamento neste horário.');
        return;
      }
    }

    console.log('Agendamento salvo:', values);
    message.success(editingAgendamento ? 'Agendamento atualizado!' : 'Agendamento criado!');
  ...
  return (
    <div className="space-y-6">
      <div>
        <Title level={2} className="!mb-2">Agendamentos</Title>
                      ...
                      <div className="w-full">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-muted-foreground">({item.duracao}min)</span>
                          </div>
                          
                          <div className="flex items-center gap-2">
                            <User size={12} className="text-muted-foreground" />
                            <span className="text-muted-foreground">{item.funcionarioNome}</span>
                          </div>
                          
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Phone size={12} className="text-muted-foreground" />
                              <span className="text-muted-foreground">{item.clienteTelefone}</span>
                            </div>
